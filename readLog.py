...
def catch_N_atoms_g09(LogFile):
    ...
    with open(LogFile) as f:
        ...
        if geometryFinalLine > geometryLine0:
...
def catch_is_linear(LogFile):
    """
        Check if molecule is linear after optimization
    """
    ...
    with open(LogFile) as f:
        ...
        for line_number, line in enumerate(f):
            if "Full point group" in line:
                point_group_line = line

        point_group = str(point_group_line.split()[3])
        if point_group == 'D*H' or point_group == 'C*V':
            is_linear = True
        else:
            is_linear = False
        return is_linear
